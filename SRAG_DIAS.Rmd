...
#https://s3-sa-east-1.amazonaws.com/ckan.saude.gov.br/SRAG/2020/INFLUD-10-08-2020.csv

library(tidyverse)
#escolhe o arquivo
file <- "/home/ochi/Documents/CPM/COVID/INFLUD-10-08-2020.csv"
...
SRAG <- read.csv2(
  ...
  #filter(CO_MUN_NOT == "3550308")
  #CHECA MUNI
  #SRAG %>% filter(ID_MUNICIP== "SAO PAULO")
...
# #diff(range(v3)) >= 6"
# #SRAG %>%
#   difftime(SRAG$DT_NOTIFIC, SRAG$DT_SIN_PRI ,units = "days")
# gc()#
#  as.Date(as.character(SRAG$DT_NOTIFIC),format="%Y/%m/%d") - as.Date(as.character(SRAG$DT_SIN_PRI),format="%Y/%m/%d")
# 
# 
# 
# difftime(strftime(SRAG$DT_NOTIFIC,"%Y/%m/%d") ,strftime(SRAG$DT_SIN_PRI,"%Y/%m/%d"),units = "days")
# 
# 
#  as.Date(as.character(SRAG$DT_NOTIFIC)) - as.Date(as.character(SRAG$DT_SIN_PRI))
# 
# as.Date(as.character(SRAG$DT_NOTIFIC)) - as.Date(as.character("5-07-20"))
#   
# ?as.Date
# 
# as.Date(strptime(SRAG$DT_NOTIFIC,'%d/%m/%Y')) - as.Date(strptime(SRAG$DT_SIN_PRI,'%d/%m/%Y'))
#rm(resultados)
...
#<- remove_outliers(as.numeric(media_tempo_sintomas_notificacao$tempo,na.rm = TRUE))  
#boxplot(remove_outliers(as.numeric(media_tempo_sintomas_notificacao$tempo,na.rm = TRUE)))
TEMPO_SINTOMAS=
...
#   quantile(media_tempo_sintomas_notificacao$tempo,na.rm = TRUE) 
#   ?quantile
# # min(media_tempo_sintomas_notificacao$tempo,na.rm = TRUE)
#  # mean(media_tempo_sintomas_notificacao$tempo,na.rm = TRUE)
# # max(media_tempo_sintomas_notificacao$tempo,na.rm = TRUE)
# 
# options(digits=2)
# TEMPO_SINTOMAS = data.frame(name = c("min","mean","max"), TEMPO_SINTOMAS = c(
#                 min(media_tempo_sintomas_notificacao$tempo,na.rm = TRUE),
#                            mean(media_tempo_sintomas_notificacao$tempo,na.rm = TRUE),
#                            max(media_tempo_sintomas_notificacao$tempo,na.rm = TRUE)
# ))
...
gc()
write.table(SRAG_DIAS, "~/SRAG_DIAS.csv", sep=";") 
```

Considerando que queremos o mais próximo de uma média, para fins de uma análise mais próxima a realidade, pegaremos então os dados do segundo ao terceiro quartil, pois como é possivel notar, a variação para o terceiro para o quarto mais do que dobra, o que indica dados mais dispersos, como o objetivo do estudo é a análise das politicas publica, por outro lado o primeiro quartil apresenta valor 0, o que dificulta uma visão mais realista, essas valores então podem por hora ser desconsiderados.